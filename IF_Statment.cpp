...
void scanToken(ifstream& inputFile, ofstream& outputFile, function<int(string)> SymbolToNum, function<string(string, string, string, string, ofstream&)> QuadsOut,
    ...
    while (inputFile >> word >> type && ending){
        ...
        if (type == "CONST" || type == "VAR" || type == "Class"){
            ...
            // if (word == "J"){
            //         while (!myStack.empty()) {
            //             cout << "Debug Stack: " + myStack.top() << endl;
            //             myStack.pop();
            //         }
            //     }
            myStack.push(word);
                    ...
                    if (relation == ">"){
                        if ((prevOp == "THEN" && type == "Rcurly") || (prevOp == "DO" && type == "Rcurly")){
                            while ((prevOp == "THEN" && type == "Rcurly") || (prevOp == "DO" && type == "Rcurly")){
                                cout << "PrevOp = THEN or DO and type == rcurly" << endl;
                                if (prevOp == "DO"){
                                    temp = myStack3.top();
                        ...
                        else if (prevOp == "Procedure" && type == "Rcurly"){
                            cout << "PrevOp = PROCEDURE and type == Rcurly" << endl;
                            myStack.pop();
                            prevOp = myStack.top();
                            cout << "prevOp: " + prevOp << endl;
                            relation = "0";
...
void QuadstoASM(ifstream& TokeninputFile, ifstream& SymbolinputFile, ofstream& ASMoutputFile){
    ...
    while (TokeninputFile >> a >> b >> c >> d){
        if (a == "+"){
            ...
            ASMoutputFile << "\tcall "  + b << endl;
        }
        else if (a == "call"){
            ASMoutputFile << "\tcall ExitProgram" << endl;
        }
        else if (a == "exit"){
                            ASMoutputFile << "\tcall ExitProgram" << endl;
        }
    ...
    ASMoutputFile << "\tExitProgram:" << endl;
                            ASMoutputFile << "\tmov eax, sys_exit" << endl;
                            ASMoutputFile << "\txor ebx, ebx      ; zero in ebx indicates success" << endl;
                            ASMoutputFile << "\tint 80h	; jmp     Again" << endl;
